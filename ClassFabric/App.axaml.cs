...
using UpdateStatus = ClassFabric.Shared.Enums.UpdateStatus;
#if DEBUG
...
using ClassFabric.Core.Controls.IconSources;
...
using ClassFabric.Shared.Protobuf.AuditEvent;
...
public partial class App : AppBase, IAppHost
{
    ...
    private void ActivatePackageType()
    {
        ...
        var packageTypeDir = Path.Combine(Environment.GetEnvironmentVariable("ClassFabric_PackageRoot") ?? exeDir,
            "PackageType");
        var fallbackPackageTypeDir = Path.Combine(exeDir, "../", "PackageType");
        var packagingRoot = Path.GetFullPath(Environment.GetEnvironmentVariable("ClassFabric_PackageRoot") ?? 
                                             Path.Combine(exeDir, "../"));
    ...
    private void ActivateAppDirectories()
    {
        ...
        CommonDirectories.AppRootFolderPath = PackagingType switch
        {
            ...
            "installer" or "deb" or "appImage" or "pkg" or "msix" => Path.GetFullPath(Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "ClassFabric", "Data")),
        ...
        if (File.Exists(Path.Combine(CommonDirectories.AppPackageRoot, "ClassFabric" + PlatformExecutableExtension)))
        {
            ExecutingEntrance =
                Path.Combine(CommonDirectories.AppPackageRoot, "ClassFabric" + PlatformExecutableExtension);
        }
        else
        {
            ExecutingEntrance = Environment.ProcessPath?.Replace(".dll", PlatformExecutableExtension) ?? "";
    ...
    {
        // HACK: 由于未知原因，在 Linux 等平台上空的 Popup 会变成一个 300x200 的空白窗口。（#1676 In ClassFabric/ClassFabric）
    ...
    internal async void ProcessUnhandledException(Exception e, bool critical=false)
    {
        ...
        IsCrashed = true;
        var safe = _isCriticalSafeModeEnabled && (!(IAppHost.TryGetService<IWindowRuleService>()?.IsForegroundWindowClassFabric() ?? false));
    ...
    private async void DesktopLifetimeOnStartup(object? sender, ControlledApplicationLifetimeStartupEventArgs e)
    {
        ...
        AppBase.CurrentLifetime = ClassFabric.Core.Enums.ApplicationLifetime.Initializing;
        ...
        {
            await CommonTaskDialogs.ShowDialog("检测到应用正在临时目录下运行", "ClassFabric正在临时目录下运行，应用设置、课表等数据很可能无法保存，或在应用退出后被自动删除。在使用本应用前，请务必将本应用解压到一个适合的位置。");
            Environment.Exit(0);
            return;
        }
        ...
        {
            var r = await CommonTaskDialogs.ShowDialog("检测到正在桌面上运行", "ClassFabric正在桌面上运行，应用设置、课表等数据将会直接存放到桌面上。在使用本应用前，请将本应用移动到一个单独的文件夹中。");
            if (r == (object)true)
            {
                Environment.Exit(0);
                return;
        ...
        {
            await CommonTaskDialogs.ShowDialog("目录权限错误", $"ClassFabric无法写入当前目录：{ex.Message}"+Environment.NewLine+Environment.NewLine+"请将本软件解压到一个合适的位置后再运行。");
            Environment.Exit(0);
            return;
        }
        ...
        IAppHost.Host = Microsoft.Extensions.Hosting.Host
            ...
            .Build();
        AppBase.CurrentLifetime = ClassFabric.Core.Enums.ApplicationLifetime.StartingOffline;
        Logger = GetService<ILogger<App>>();
        Logger.LogInformation("ClassFabric {}", AppVersionLong);
        ...
        if (!string.IsNullOrWhiteSpace(ApplicationCommand.ImportV1) || !string.IsNullOrWhiteSpace(ApplicationCommand.ImportV2))
        {
            ...
            {
                await dtWindow.PerformClassFabricImport(ApplicationCommand.ImportV1, (ImportEntries)entries);
            }
            else
            {
                await dtWindow.PerformClassFabric2Import(ApplicationCommand.ImportV2, (ImportEntries)entries);
            }
            return;
        }
        ...
        {
            await GetService<DiagnosticService>().ExportDiagnosticData(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), $"ClassFabric_DiagnosticData_{DateTime.Now:yy-MMM-dd_HH-mm-ss}.zip"), false);
        }
        ...
        mw.StartupCompleted += (o, args) =>
        {
            ...
            {
                PlatformServices.DesktopToastService.ShowToastAsync("配置文件损坏", "ClassFabric 部分配置文件已损坏且无法加载，这些配置文件已恢复至默认值。点击此消息以查看详细信息和从过往备份中恢复配置文件。", () => GetService<IUriNavigationService>().NavigateWrapped(new Uri("classisland://app/config-errors")));
            }
                    ...
                    {
                        {
                            ...
                            () => GetService<IUriNavigationService>()
                                .NavigateWrapped(new Uri("classisland://app/settings/classfabric.plugins"))
                        },
                        {
                            "管理异常插件行为",
                            () => GetService<IUriNavigationService>()
            ...
            AppBase.CurrentLifetime = ClassFabric.Core.Enums.ApplicationLifetime.Running;
        ...
        // 竖版课表（Beta）启动接线：必须在 MainWindow.Show 之后初始化，避免竖窗先于主窗出现
        GetService<VerticalScheduleService>().Initialize();
    ...
    {
        if (CurrentLifetime == ClassFabric.Core.Enums.ApplicationLifetime.Stopping)
        {
            return;
        }
        _ = Dispatcher.UIThread.InvokeAsync(() =>
        {
            ...
            CurrentLifetime = ClassFabric.Core.Enums.ApplicationLifetime.Stopping;
