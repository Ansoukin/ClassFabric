...
public partial class SettingsWindowNew : ViewBase, IFANavigationPageFactory
{
    ...
    private async Task UpdateEchoCaveAsync()
    {
        ...
        {
            var sayings = await AssetLoader.ReadAllTextAsync(new Uri("avares://ClassFabric/Assets/Tellings.txt"));
            if (_resourcesReleased)
            {
                return;
            }
        ...
        catch (Exception ex)
        {
            ...
            if (info.Id != ErrorPageId)
            {
                await CoreNavigate(SettingsWindowRegistryService.Registered.First(x => x.Id == ErrorPageId),
                    new Uri("classfabric://app/settings/_error?error=true"));
            }
        }
        //finally
        //{
    ...
    public override async void Open(ViewBase? owner = null)
    {
        ...
        TutorialService.PushToNextSentenceByTag("classfabric.settingsWindow.open");
        base.Open(owner);
    }
    ...
    {
        IAppHost.GetService<IUriNavigationService>().NavigateWrapped(new Uri("classfabric://app/settings/management?ci_keepHistory=true"));
    }
