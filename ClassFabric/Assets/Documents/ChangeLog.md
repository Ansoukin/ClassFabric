...
2.1 - KapoKali（卡利白） 的新增功能。

## 翻新与迎新

当您在长假长时间没有启动 ClassFabric 后，再次启动 ClassFabric 时应用会询问您是否翻新部分设置以适应新学期。同时您也可以启用迎新模式，迎新模式会在翻新的基础上重置教程状态，以便新同学上手应用。
...
ClassFabric 现在引入了教程系统，您可以跟着教程手把手地上手 ClassFabric。
...
现在 ClassFabric 支持通过编辑模式所见即所得地编辑主界面上的组件，在各个组件行和容器组件间直接拖动组件，并快捷地调整各个设置。
