...
public static class ShortcutHelpers
{
    ...
    {
        await using var stream = AssetLoader.Open(new Uri("avares://ClassFabric/Assets/ShortcutTemplates/ClassSwap.url"));
    ...
    public static async Task CreateFreedesktopShortcutAsync(string path = "", bool isAutostart = false)
    {
        ...
        {
            File.Copy("/usr/share/applications/cn.classfabric.app.desktop", path, true);
        ...
        var raw = await AssetLoader.ReadAllTextAsync(
            new Uri("avares://ClassFabric/Assets/ShortcutTemplates/cn.ClassFabric.app.desktop"));
    ...
    public static async Task CopyFreeDesktopIconAsync()
    {
        if (AppBase.Current.PackagingType is "folder" or "folderClassic") // 仅在绿色版下才需要手动复制图标，安装版应当由安装程序将图标复制到系统目录。)
        {
            ...
            await using var src = AssetLoader.Open(new Uri("avares://ClassFabric/Assets/FreedesktopIcons/AppLogo@128w.png"));
            await using var file = File.OpenWrite(Path.Combine(iconsDir, "classfabric.png"));
