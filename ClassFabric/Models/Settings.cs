...
using ClassFabric.Core.Models.Weather;
using ClassFabric.Shared;
using ClassFabric.Shared.Abstraction.Models;
using ClassFabric.Shared.Enums;
using ClassFabric.Shared.Models.Notification;
using ClassFabric.Models.AllContributors;
using CommunityToolkit.Mvvm.ComponentModel;
using Microsoft.Extensions.Logging;
using Octokit;
using ClassFabric.Core.Models;
...
using ClassFabric.Models.Refreshing;
using ClassFabric.Services;

namespace ClassFabric.Models;
...
public class Settings : ObservableRecipient, ILessonControlSettings, INotificationSettings, IGlobalSpeechSettings
{
    ...
    private string _selectedChannel = "https://install.appcenter.ms/api/v0.1/apps/hellowrc/classfabric/distribution_groups/public";
    ...
    private VerticalScheduleSettings _verticalScheduleSettings = new();

    public Settings()
    {
        // 订阅初始实例，保证未持久化（全新配置）时子属性变更也能转发保存
        _verticalScheduleSettings.PropertyChanged += VerticalScheduleSettingsOnPropertyChanged;
    }
    ...
    [JsonPropertyName("SettingsOverlay")] // ClassFabric 1.x 名称
    ...
    private string _selectedSpeechProvider = "classfabric.speech.edgeTts";
    private bool _isThemeWarningVisible = true;
    private string _weatherIconId = "classfabric.weatherIcons.lucide";
    ...
    /// 竖版课表（Beta）设置
    /// </summary>
    public VerticalScheduleSettings VerticalScheduleSettings
    {
        get => _verticalScheduleSettings;
        set
        {
            if (Equals(value, _verticalScheduleSettings)) return;
            if (_verticalScheduleSettings != null)
                _verticalScheduleSettings.PropertyChanged -= VerticalScheduleSettingsOnPropertyChanged;
            _verticalScheduleSettings = value;
            if (_verticalScheduleSettings != null)
                _verticalScheduleSettings.PropertyChanged += VerticalScheduleSettingsOnPropertyChanged;
            OnPropertyChanged();
        }
    ...
    private void VerticalScheduleSettingsOnPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        // 子属性变更转发为容器级根通知，复用 SettingsService 根级 PropertyChanged 自动保存链路
        OnPropertyChanged(nameof(VerticalScheduleSettings));
    }
...
/// <summary>
/// 竖版课表（Beta）设置。嵌套设置对象，子属性变更经 Settings.VerticalScheduleSettingsOnPropertyChanged 转发保存。
/// 容器与子属性均不加 [SettingsInfo]。
/// </summary>
public partial class VerticalScheduleSettings : ObservableObject
{
    /// <summary>
    /// 竖版课表总开关
    /// </summary>
    ...
    /// </summary>
    [ObservableProperty]
    private int _dockingEdge = 1;

    /// <summary>
    /// 竖条纵向位置微调，正值向下，单位物理像素
    /// </summary>
    [ObservableProperty]
    ...
    /// </summary>
    [ObservableProperty]
    private bool _isBreakingReminderEnabled = false;

    /// <summary>
    /// 下课提示「现在是下课时间」画面的停留秒数
    /// </summary>
    [ObservableProperty]
    private int _breakingReminderBreakLabelSeconds = 3;

    /// <summary>
    /// 下课提示「下一节课是」与下节课程全名两画面的停留秒数
    /// </summary>
    [ObservableProperty]
    private int _breakingReminderNextLessonSeconds = 3;

    /// <summary>
    /// 下课提示两轮之间显示课表的秒数
    /// </summary>
    [ObservableProperty]
    private int _breakingReminderScheduleDisplaySeconds = 10;

    /// <summary>
    /// 下课提示最多重复轮数
    /// </summary>
    [ObservableProperty]
    private int _breakingReminderMaxRounds = 2;

    /// <summary>
    /// 临近上课时在竖条上轮播上课预告
    /// </summary>
    [ObservableProperty]
    private bool _isOnClassReminderEnabled = false;

    /// <summary>
    /// 上课预告两个画面的停留秒数（共用）
    /// </summary>
    [ObservableProperty]
    private int _onClassReminderPageSeconds = 3;

    /// <summary>
    /// 上课预告两轮之间显示课表的秒数
    /// </summary>
    [ObservableProperty]
    private int _onClassReminderScheduleDisplaySeconds = 10;

    /// <summary>
    /// 上课预告最多重复轮数
    /// </summary>
    [ObservableProperty]
    private int _onClassReminderMaxRounds = 2;

    /// <summary>
    /// 竖条整体缩放：在全局界面缩放基础上叠加，格高、字号与宽度长宽等比
    /// </summary>
    [ObservableProperty]
    private double _zoom = 1.0;

    /// <summary>
    /// 启用竖条独立圆角：开启后使用独立圆角值，不再跟随全局界面圆角
    /// </summary>
    [ObservableProperty]
